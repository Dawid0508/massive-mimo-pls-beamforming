...
print_scenario_snr('title', 'Artificial noise @ 6 GHz', ...
...
for l_idx = 1:length(L_values)
    ...
    for it = 1:numIter
        [SR_no, SR_an] = run_AN_trial(sv, fc, Nt, K, num_eve, ...
                                      P_rx, cdl_tag, phi_fixed, noise_var);
        acc_no = acc_no + SR_no;
        acc_an = acc_an + SR_an;
    end
...
for ph_idx = 1:length(phi_values)
    ...
    for it = 1:numIter
        [~, SR_an] = run_AN_trial(sv, fc, Nt, K, L_fixed, ...
                                  P_rx, cdl_tag, phi_values(ph_idx), noise_var);
        acc = acc + SR_an;
    end
...
legend('Plain ZF', 'ZF + AN', 'Location', 'SouthWest');
...
c = pls_colors();
xline(phi_fixed, ':', sprintf('\\phi = %.1f', phi_fixed), 'Color', c.beam, 'LineWidth', 1.5);

sgtitle(sprintf('Artificial Noise @ 6 GHz (CDL-A, K = %d, d = %d m, received SNR = %d dB)', ...
    K, dist, SNR_rx_dB));
...
function [SR_no_AN, SR_with_AN] = run_AN_trial(sv, fc, Nt, K, num_eve, ...
                                               P_rx, cdl_tag, phi, noise_var)
    theta_bobs = -60 + 120*rand(1, K);
    theta_eves = -90 + 180*rand(1, num_eve);

    H = zeros(Nt, K);
    for k = 1:K
        H(:, k) = channel_3gpp_ula(sv, fc, theta_bobs(k), cdl_tag);
    end
    G = zeros(Nt, num_eve);
    for e = 1:num_eve
        G(:, e) = channel_3gpp_ula(sv, fc, theta_eves(e), cdl_tag);
    end
