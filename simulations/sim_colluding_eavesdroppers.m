...
% -------------------------------------------------------------------------
% ZAWARTY FIX: Optymalizacja obiektów nrCDL, poprawny model fading'u, 
% klarowna definicja parametru rho_tx (Transmit SNR) oraz wyliczenie 
% rzeczywistego Received SNR u klientów.
% =========================================================================
pls_startup();
...
Rx_SNR_dB    = zeros(2, 1); % Wektor na zapisanie średniego Received SNR
max_L = max(L_values);
...
for b = 1:2
    ...
    % Wyliczenie wektorowe fizycznego tłumienia dla zadanej odległości
    [PL_lin, PL_dB] = compute_fspl(dist, fc); 
    
    % Wyliczenie średniego docelowego Received SNR
    % Wzór: (P_tx * Zysk antenowy Nt) / Tłumienie trasy
    ...
    % INICJALIZACJA KANAŁÓW POZA PĘTLĄ MONTE CARLO (Optymalizacja prędkości)
    cdl_b = cell(K, 1);
    for k = 1:K
        cdl_b{k} = create_base_cdl(Nt, fc);
    end
    cdl_e = cell(max_L, 1);
    for e = 1:max_L
        cdl_e{e} = create_base_cdl(Nt, fc);
    end
    ...
    for l_idx = 1:length(L_values)
        ...
        for it = 1:numIter
            % Losowanie topologii w sektorze 120 stopni dla każdej iteracji
            ...
            % --- KANAŁY BOBÓW ---
            for k = 1:K
                release(cdl_b{k}); % Zwalniamy obiekt przed modyfikacją
                cdl_b{k}.TransmitArrayOrientation = [-theta_bobs(k); 0; 0];
                cdl_b{k}.Seed = iter_seed;
                [pg_b, ~] = cdl_b{k}();
                
                hb = squeeze(sum(pg_b, 2)); hb = hb(:);
                % Aplikacja tłumienia z zachowaniem naturalnej wariancji
            ...
            for e = 1:num_eve
                ...
                G_eff(:, e) = sqrt(1 / PL_lin) * he;
            end
            
            % --- ZERO-FORCING (Z DROBNĄ REGULARYZACJĄ NUMERYCZNĄ) ---
            W_raw = H_eff * pinv(H_eff' * H_eff + 1e-9 * eye(K));
            W = W_raw / norm(W_raw, 'fro'); % Normalizacja macierzowa całej mocy
            
            R_inv = eye(num_eve);
            R_b = zeros(K, 1);
            R_e = zeros(K, 1);
            
            % --- EWALUACJA POJEMNOŚCI ---
            for k = 1:K
                % Pojemność Boba
                ...
                % Pojemność koalicji Ew (Maximum Ratio Combining)
                h_ek = sqrt(P_tx) * G_eff' * W(:, k);
                R_e(k) = log2(1 + real(h_ek' * R_inv * h_ek));
            end
            
            % Obliczenie metryk PLS
            R_s = max(0, R_b - R_e);
            rs_sum = sum(R_s);
            SR_acc = SR_acc + rs_sum;
            
            % Obliczenie sprawiedliwości z zabezpieczeniem przed dzieleniem przez zero
            if sum(R_s.^2) > 0
...
% =========================================================================
% Dynamiczne etykiety legendy prezentujące pasmo, Nt i wynikowy Rx SNR
...
% =========================================================================
% FUNKCJA POMOCNICZA: BAZOWY KANAŁ CDL
% =========================================================================
function cdl = create_base_cdl(Nt, fc)
    cdl = nrCDLChannel;
    cdl.DelayProfile = 'CDL-A';
    
    % Zróżnicowanie Delay Spread dla sub-6 GHz i mmWave
    if fc < 10e9
