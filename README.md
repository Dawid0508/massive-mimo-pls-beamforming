...
| **Phased Array System Toolbox** | `phased.ULA`, `phased.SteeringVector`, `physconst`, `step(sv,...)` in `setup_ula` / `channel_3gpp_ula` |

Install Phased Array: **Home → Add-Ons → Get Add-Ons** → search *Phased Array System Toolbox* → Install → restart MATLAB.
...
|--------|---------------|
| `Ghz6_band_vs_mmWave_band.m` | `fig_baseline_6GHz_vs_28GHz.png` |
| `sim_moving_bob.m` | `fig_moving_bob.png` |
| `sim_pilot_contamination.m` | `fig_pilot_contamination.png` |
| `sim_colluding_eavesdroppers.m` | `fig_colluding_eavesdroppers.png` |
| `sim_artificial_noise.m` | `fig_artificial_noise.png` |
| `sim_location_error.m` | `fig_location_error.png` |
| `sim_phase_noise.m` | `fig_phase_noise.png` |
| `sim_spatial_correlation.m` | `fig_spatial_correlation.png` |
| `sim_fairness_normalization.m` | `fig_fairness_normalization.png` |
| `sim_channel_hardening.m` | `fig_channel_hardening.png` |
...
- **Secrecy rate:** `secrecy_rate` — Wyner clamp at zero.
- **Plots:** Dark background (`#1a1a2e`), light axes; Bob green, Eve salmon. `results/*.log` is git-ignored; PNGs are committed.
